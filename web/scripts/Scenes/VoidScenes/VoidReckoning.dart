...
class VoidReckoning extends Scene {

    VoidReckoning(Session session) : super(session);
...
void renderContent(Element div) {
    ...
    void yellowLawnRing(Element div) {
        Relationship r = session.players[0].getRelationshipWith((session as VoidSession).metaPlayer);
        if(!session.players[0].dead && (r.value >0 || this.session.janusReward || (session.session_id == 4037))) {
            ...
            if((session as VoidSession).metaPlayer != session.mutator.metaHandler.jadedResearcher && (session as VoidSession).metaPlayer != session.mutator.metaHandler.authorBot) metaScene(div);
    ...
    void metaScene(Element div) {
        ...
        VoidSession d = session as VoidSession;
